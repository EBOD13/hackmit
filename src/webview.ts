...
export function setupExpressRoutes(server: AppServer): void {
  ...
  app.get('/webview', async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (userId && (server as any).database) {
        ...
        try {
          ...
          if (currentQuest) {
            questTemplate = await database.getQuestTemplateById(currentQuest.quest_template_id);
          }
        } catch (error) {
  ...
  app.post('/api/quest/new', async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.authUserId;
      if (!userId) {
      ...
      if (existingQuest) {
        return res.json({ success: false, message: 'You already have an active quest!' });
      }
      ...
      if (userLocation && placesService) {
        const questLocations = await placesService.findQuestLocations(userLocation.lat, userLocation.lng);
        if (questLocations.length > 0) {
          const randomLocation = questLocations[Math.floor(Math.random() * questLocations.length)];
          const questData = placesService.generateQuestDescription(randomLocation.place, randomLocation.questType);
      ...
      if (questTemplate) {
        const activeQuest = await database.createActiveQuest(userId, questTemplate.id);
        if (activeQuest) {
  ...
  app.post('/api/quest/complete', async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.authUserId;
      if (!userId) {
        return res.json({ success: false, message: 'Not authenticated' });
      ...
      if (!activeQuest) {
        return res.json({ success: false, message: 'No active quest to complete' });
      }

      // Get quest template to get correct points
      const questTemplate = await database.getQuestTemplateById(activeQuest.quest_template_id);
  ...
  app.get('/api/quest/current', async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.authUserId;
      if (!userId) {
        return res.json({ success: false, message: 'Not authenticated' });
